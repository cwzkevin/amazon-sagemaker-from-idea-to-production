...
from model.model_pytorch import GraphSAGEModel, NodeClassification
...
if __name__ == '__main__':
    ...
    train_validation_df = pd.read_csv(os.path.join(args.training_dir, args.features_train), header=0, index_col=0)
    test_df = pd.read_csv(os.path.join(args.training_dir, args.features_test), header=0, index_col=0)
    
    
    src_dst_df = pd.read_csv(os.path.join(args.training_dir, args.source_destination_node_index), header=0)

    logging.info("Further slice the train dataset into train and validation datasets.")
    train_df, validation_df = train_test_split(train_validation_df, test_size=0.2, random_state=42, stratify=train_validation_df[args.target_column].values)
    ...
    features = torch.FloatTensor(train_val_test_df.drop(['node', args.target_column], axis=1).to_numpy()) 
    ...
    labels = torch.LongTensor(train_val_test_df[args.target_column].values)
    n_classes = train_val_test_df[args.target_column].nunique()
    logging.info(f"Number of classes = {n_classes}.")

    graph.ndata['feat'] = features

    logging.info("Initializing Model")
    gconv_model = GraphSAGEModel(num_feats, args.n_hidden, n_classes, args.n_layers, F.relu, args.dropout, args.aggregator_type)
    
    # Node classification task
    model = NodeClassification(gconv_model, args.n_hidden, n_classes)
    ...
    model, metric_table, y_true, y_pred, y_pred_prob = train(model, optimizer, graph, features, labels, train_mask, val_mask, test_mask, args.n_epochs)
    logging.info("Finished Model training")

    logging.info("Saving model")
    save_model(graph, model, args.model_dir)
    
    logging.info("Saving model predictions for test data")
    save_prediction(y_true, y_pred, y_pred_prob, args.output_dir, args.predictions_file_name)
